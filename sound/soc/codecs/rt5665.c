...
static struct {
	...
	unsigned int bias;
	unsigned int volumegain;
} hp_gain_table[] = {
	{0x0000, 0x000d, 0, 6, 0x57}, /* 8 ohm */
	{0x000e, 0x0019, 0, 6, 0x57}, /* 16 ohm */
	{0x001a, 0x0035, 0, 6, 0x57}, /* 32 ohm */
	{0x0036, 0x0067, 0, 1, 0x57}, /* 64 ohm */
	{0x0068, 0xffff, 0, 1, 0x57}, /* > 100 ohm */
};
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (rt5665->impedance_gain_map == true) {
		...
		snd_soc_update_bits(codec, RT5665_HP_IMP_GAIN_1,
			0x7f7f, rt5665->impedance_vol_gain<<8 | rt5665->impedance_vol_gain);
	}
...
static unsigned int rt5665_imp_detect(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < ARRAY_SIZE(hp_gain_table); i++) {
		...
		rt5665->impedance_vol_gain = hp_gain_table[i].volumegain;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((rt5665->jack_type & SND_JACK_HEADSET) && (rt5665->jack_type & SND_JACK_BTN_0))
		value = 1;
...
static int rt5665_probe(struct snd_soc_codec *codec)
{
	...
	if (IS_ERR(audio)) {
		dev_err(codec->dev, "Failed to create class audio\n");
		ret = -ENODEV;
	...
	if (IS_ERR(earjack)) {
		dev_err(codec->dev, "Failed to create device earjack\n");
		ret = -ENODEV;
	...
	if (ret) {
		dev_err(codec->dev, "Failed to create device file in %s\n",
	...
	dev_set_drvdata(earjack, rt5665);
	return 0;
...
static int rt5665_parse_dt(struct rt5665_priv *rt5665, struct device *dev)
{
	...
	rt5665->pdata.ldo1_en = of_get_named_gpio(dev->of_node,
		"realtek,ldo1_en", 0);
	rt5665->pdata.gpio_3p3_en = of_get_named_gpio(dev->of_node,
		"realtek,gpio_3p3_en", 0);
	...
	if (!of_property_read_u32_array(dev->of_node, "imp_table", data,
		...
		for (i = 0; i < len; i++) {
			hp_gain_table[i].min = data[i * 5];
			hp_gain_table[i].max = data[(i * 5) + 1];
			hp_gain_table[i].gain = data[(i * 5) + 2];
			hp_gain_table[i].bias = data[(i * 5) + 3];
			hp_gain_table[i].volumegain = data[(i * 5) + 4];
			pr_debug("%s: min=%d,max=%d ==> gain=%d,volumegain=0x%x\n", __func__,
				hp_gain_table[i].min, hp_gain_table[i].max,
				hp_gain_table[i].gain, hp_gain_table[i].volumegain);
		}
		...
		rt5665->impedance_vol_gain = 0x57;
	} else {
...
rt5665_i2c_probe_retry:
	...
	if (gpio_is_valid(rt5665->pdata.gpio_3p3_en)) {
		if (gpio_request(rt5665->pdata.gpio_3p3_en, "rt5665_3p3_en"))
			dev_err(&i2c->dev, "Fail gpio_request rt5665_3p3\n");
		else if (gpio_direction_output(rt5665->pdata.gpio_3p3_en, 1))
			dev_err(&i2c->dev, "Fail gpio_direction rt5665_3p3\n");
		else
			dev_err(&i2c->dev, "gpio_direction rt5665_3p3 is success\n");
	}
	...
	if (IS_ERR(rt5665->regmap)) {
		...
		panic("Failed to allocate register map");
#endif
		return ret;
	}
	...
	if (val != RT5665_6_8_DEVICE_ID) {
		...
		if (retry_cnt < max_retry_cnt) {
			retry_cnt++;
			goto rt5665_i2c_probe_retry;
		}
#ifdef CONFIG_SEC_SND_DEBUG
		panic("fail to read device ID from rt5665");
#endif
		return -ENODEV;
	}
